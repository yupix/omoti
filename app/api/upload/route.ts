...
export async function POST(req: NextRequest) {
    ...
    const temporaryPath = path.join(uploadDir, `${uniqueFilename}.part`);
    let bytes = 0;
    try {
...
export async function GET() {
    try {
        ...
        const mediaFiles = files.filter(f => /\.(jpg|jpeg|png|gif|mp4|webm|mp3|wav|ogg|m4a|aac|flac|mkv|mov|webp|svg|bmp|avif|psd)$/i.test(f));
